...
- **Preparacao operacional (pre-deploy)**: sem Relation registrada, o gate bloqueia logs de sessao e `/meu_perfil` ate para o admin. Antes do deploy, o mantenedor roda `python scripts/bootstrap_admin_relation.py` (relatorio/dry-run, inclui inventario de legado em quarentena) e depois `--apply` para cadastrar explicitamente a Relation do admin (status `active`, consentimento `granted`) e validar o gate; o script e idempotente e nao cria vinculos automaticamente, porque consentimento continua sendo uma decisao explicita. A reclassificacao, copia ou remocao do legado exige decisao registrada do mantenedor.
- **Aceite local**: 789 testes offline aprovados na preparacao do PR (767 do checkpoint mais sentinelas do caminho do volume persistente e do bootstrap da Relation do admin); sentinelas cobrem duas Relations, duas instancias, escopo ausente/invalido, revogacao, ausencia de escrita no caminho legado, precedencia do volume e idempotencia do cadastro. Sem chamadas LLM, Telegram, banco de producao, push ou deploy neste checkpoint.
