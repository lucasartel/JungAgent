...
- **Consolidacao de memorias**: a geracao do `profile.md` no namespace da Relation (promessa do `/meu_perfil`) roda diariamente via `memory_consolidation_scheduler` (agendado no boot, sem depender de trigger manual); usuarios sem Relation ativa sao pulados com registro proprio (`relation_scope_required_for_consolidation`), sem poluir o log de erro.
